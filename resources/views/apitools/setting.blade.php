...
<div class="content">
    <div class="row">
       <div class="col-sm-6">
            <div class="panel panel-default">
                <div class="panel-heading">
                    <h4 class="panel-title">Payout Call Back</h4>
                </div>
                <form id="payoutcallbackForm" action="{{route('profileUpdate')}}" method="post">
                    {{ csrf_field() }}
                    <input type="hidden" name="id" value="{{Auth::user()->id}}">
                    <div class="panel-body" style="padding:16px">
                        <div class="form-group">
                            <textarea name="payoutcallbackurl" class="form-control" cols="30" rows="3" required placeholder="Enter Callback Url">{{Auth::user()->payoutcallbackurl ?? ""}}</textarea>
                        </div>
                    </div>
                    <div class="panel-footer">
                        <button class="btn bg-slate btn-raised legitRipple pull-right" type="submit" data-loading-text="<i class='fa fa-spin fa-spinner'></i> Updating...">Update Info</button>

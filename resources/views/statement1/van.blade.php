        ...
        <div class="col-sm-12">
            <div class="panel panel-default">
                <div class="panel-heading">
                    <h4 class="panel-title">VAN  List</h4>
                </div>
        ...
        <div class="modal-content">
            ...
            <form id="editUtiidForm" action="{{route('statementUpdate')}}" method="post">
                <div class="modal-body">
                    ...
                    <input type="hidden" name="actiontype" value="contact">
                    {{ csrf_field() }}
                    
                    <div class="form-group">
                        <label>Callback Url</label>
                        <input type="text" name="contactId" class="form-control" placeholder="Enter id" required="">
                    </div>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-default btn-raised legitRipple" data-dismiss="modal" aria-hidden="true">Close</button>
...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        var url = "{{url('statement/fetch')}}/vanaccountlist/{{$id}}";
        var onDraw = function() {
        };
        var options = [
            { "data" : "name",
            ...
            { "data" : "bank",
                render:function(data, type, full, meta){
                    return `VAN Number- `+full.vanAccount;
                }
            }
        ];
    ...
    function viewFullData(id){
        $.ajax({
            url: `{{url('statement/fetch')}}/contactstatement/`+id+`/view`,
            type: 'post',
            headers: {
    ...
    function editUtiid(id, contactId){
        $('#editModal').find('[name="id"]').val(id);
        $('#editModal').find('[name="contactId"]').val(contactId);

...
// 			case 'matmstatement':
// 				$data->where('rtype', 'main')->where('aepstype', 'MATM')->where('status', '!=', 'initiated')->where('amount','!=','0');
// 				break;	
	...
	public function transactionStatus(Request $post)
    {
		...
		switch ($post->type) {
			...
			default:
				return response()->json(['statuscode' => "ERR", "message" => "Status Not Allowed"], 200);
				break;
		}

		if(!$report || !in_array($report->status , ['pending', 'success', 'approved', 'initiated'])){
			return response()->json(['statuscode' => "ERR", "message" => "Status Not Allowed, Transaction is ".$report->status], 200);
		}

		if($post->type == "aeps" && (!$report || !in_array($report->status , ['pending']))){
			return response()->json(['statuscode' => "ERR", "message" => "Aeps Status Not Allowed"], 200);
		}
		
		if($post->type == "matm" && (!$report || !in_array($report->status , ['pending', 'initiated']))){
			return response()->json(['statuscode' => "ERR", "message" => "Matm Status Not Allowed"], 200);
		}
		...
		switch ($post->type) {
			case 'recharge':
				switch ($report->api->code) {
					...
					default:
						return response()->json(['statuscode' => "ERR", "message" => "Recharge Status Not Allowed"], 200);
						break;
				}
		...
		if($result['response'] != ''){
				...
				switch ($post->type) {
                    ...
                    case 'aeps':
						...
						if($report->status == "pending" && $update['status'] == "complete"){
                            ...
                            }elseif($report->amount>3000 && $report->amount<=2000){
                                $provider = Provider::where('recharge1', 'aeps7')->first();
                            }elseif($report->amount>2000 && $report->amount<=5000){
						...
						if(in_array($report->status, ["pending", 'initiated']) && $update['status'] == "complete"){
                            ...
                            if($myreport->amount > 0){
	                           if($myreport->amount >= 100 && $myreport->amount <= 500){
	                            ...
	                            }elseif($myreport->amount > 2000 && $myreport->amount <= 5000){
			...
			return response()->json(['status' => "Status Not Fetched , Try Again."], 200);
		}
	}
}
